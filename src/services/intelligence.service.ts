...
function isOpenTask(task: TaskDto) {
  return task.status !== 'completed';
}

export function isOverdue(task: TaskDto) {
  return task.date < todayStr() && isOpenTask(task);
}

function scoreTask(task: TaskDto) {
  let score = 0;
...
export function getNextBestTask(tasks: TaskDto[]): NextBestTaskDto | null {
  ...
  } else if (best.date === todayStr() && best.priority === 'high') {
    reason = 'High priority + due today';
  } else if (best.date === todayStr()) {
    reason = 'Due today';
...
export function analyzeWorkload(tasks: TaskDto[], date: string): WorkloadAnalysisDto {
  ...
  let status: WorkloadAnalysisDto['status'] = 'normal';
...
export function generateDailySummary(tasks: TaskDto[], date: string): DailySummaryDto {
  const dayTasks = tasks.filter((task) => task.date === date);
  ...
  const productivityScore = Math.min(
    100,
    Math.round(completionRate * 0.6 + (Math.min(completedTime, 8) / 8) * 100 * 0.4),
  );
...
export function buildInsights(tasks: TaskDto[]): InsightsDto {
