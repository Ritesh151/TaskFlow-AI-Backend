...
exports.getBrainDashboard = getBrainDashboard;
exports.getBrainGraph = getBrainGraph;
exports.getBrainNote = getBrainNote;
exports.createBrainNote = createBrainNote;
exports.updateBrainNote = updateBrainNote;
...
const BRAIN_CATEGORIES = ['idea', 'bug', 'learning', 'snippet', 'thought', 'research'];
...
function parseHashtags(text) {
    return uniqueStrings([...String(text || '').matchAll(/#([a-z0-9][a-z0-9-]*)/gi)].map((match) => normalizeKeyword(match[1] ?? '')));
}
...
function inferCategory(title, content, tags = []) {
...
function extractKeywords(input, extraTerms = []) {
...
function extractTechnologies(keywords) {
...
function buildTaskKeywords(task) {
    return extractKeywords([task.taskName, task.client, ...(task.technologies ?? []), ...(task.tags ?? [])].join(' '), [...(task.technologies ?? []), ...(task.tags ?? [])]);
}
function noteSearchText(note) {
    return normalizeText([note.title, note.content, note.category, ...note.tags, ...note.keywords].join(' '));
}
function normalizeTags(tags) {
    return clampArray(uniqueStrings(tags
        .map((tag) => normalizeKeyword(tag))
        .filter((tag) => Boolean(tag) && tag.length <= 24)), 10);
}
function serializeRawNote(note) {
    const category = BRAIN_CATEGORIES.includes(note.category)
        ? note.category
        : 'thought';
    return {
        brainId: note.id,
...
function buildRelatedTaskLink(note, task) {
...
function buildRelatedNoteLink(source, candidate) {
...
function generateRelatedNotes(note, notes, tasks) {
    ...
    return {
        relatedNotes: clampArray(relatedNotes, 6),
        relatedTasks: clampArray(relatedTasks, 4),
    };
}
function syncBrainNotes(notes, tasks) {
...
function sortNotes(notes, sort = 'newest') {
    const copy = [...notes];
...
function detectTopicClusters(notes) {
    const frequency = new Map();
    ...
    const keywordClusters = repeatedKeywords.map(([keyword, count]) => {
        ...
        return {
            ...
            categories: uniqueStrings(clusterNotes.map((note) => note.category)),
            noteIds: clusterNotes.map((note) => note.brainId),
            keywords: clampArray(uniqueStrings(clusterNotes.flatMap((note) => note.keywords).filter((item) => item !== keyword)), 5),
...
function buildGrowth(notes, days = 10) {
    const rows = [];
...
function buildKnowledgeStreak(notes) {
...
function buildInsights(notes) {
    ...
    const topTechnologies = clampArray([...notes.reduce((map, note) => {
        ...
        .sort((left, right) => right[1] - left[1] || left[0].localeCompare(right[0]))
        .map(([name, count]) => ({ name, count })), 6);
    const recentTopics = clampArray(uniqueStrings(sortNotes(notes, 'newest').slice(0, 8).flatMap((note) => note.keywords)), 8);
    return {
        totalNotes,
        linkedNotes,
        ideasCaptured,
...
async function loadSyncedNotes(userId) {
    const [notes, tasks] = await Promise.all([
        prisma_1.prisma.brainNote.findMany({
            where: { userId },
            orderBy: [{ updatedAt: 'desc' }],
        }),
        (0, task_service_1.listTasks)(userId),
    ]);
...
async function getBrainDashboard(userId, options = {}) {
    return getDashboardData(userId, options);
}
async function getBrainGraph(userId) {
    const syncedNotes = await loadSyncedNotes(userId);
...
async function getBrainNote(userId, noteId) {
    const notes = await loadSyncedNotes(userId);
    const note = notes.find((item) => item.brainId === noteId);
    if (!note) {
        throw (0, errors_1.notFound)('Note not found');
    }
    return note;
}
async function createBrainNote(userId, input) {
...
async function updateBrainNote(userId, noteId, input) {
    const existing = await prisma_1.prisma.brainNote.findFirst({
        where: {
            id: noteId,
            userId,
        },
    ...
    const finalTitle = deriveTitle(title, content);
    const keywords = extractKeywords(`${finalTitle} ${content}`, tags);
    await prisma_1.prisma.brainNote.update({
...
async function toggleBrainFavorite(userId, noteId, value) {
    const existing = await prisma_1.prisma.brainNote.findFirst({
        where: {
            id: noteId,
            userId,
...
async function toggleBrainPin(userId, noteId, value) {
    const existing = await prisma_1.prisma.brainNote.findFirst({
        where: {
            id: noteId,
            userId,
...
async function deleteBrainNote(userId, noteId) {
    const existing = await prisma_1.prisma.brainNote.findFirst({
        where: {
            id: noteId,
            userId,
    ...
    await prisma_1.prisma.brainNote.delete({
        where: { id: existing.id },
    });
}
