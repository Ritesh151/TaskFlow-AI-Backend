...
exports.startBreakForUser = startBreakForUser;
exports.endBreakForUser = endBreakForUser;
exports.startDeepWorkForUser = startDeepWorkForUser;
exports.endDeepWorkForUser = endDeepWorkForUser;
...
function castBreaks(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value.map((item) => {
        const breakEntry = item;
        return {
            breakId: String(breakEntry.breakId ?? (0, node_crypto_1.randomUUID)()),
            start: String(breakEntry.start ?? new Date().toISOString()),
            end: breakEntry.end ? String(breakEntry.end) : null,
            type: breakEntry.type ?? 'custom',
        };
    });
}
function castDeepWorkSessions(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value.map((item) => {
        const session = item;
        return {
            sessionId: String(session.sessionId ?? (0, node_crypto_1.randomUUID)()),
            start: String(session.start ?? new Date().toISOString()),
            end: session.end ? String(session.end) : null,
        };
    });
}
function castTimeline(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    return value.map((item) => {
...
function createDefaultRecord(date) {
    return {
        attendanceId: (0, node_crypto_1.randomUUID)(),
        date,
        checkIn: null,
        checkOut: null,
        status: 'absent',
...
function computeRecord(record) {
    ...
    const totalElapsedMinutes = (0, dates_1.minutesBetween)(record.checkIn, checkOut);
    const totalBreakMinutes = record.breaks.reduce((sum, breakEntry) => sum + (0, dates_1.minutesBetween)(breakEntry.start, breakEntry.end || now), 0);
    const deepWorkMinutes = record.deepWorkSessions.reduce((sum, session) => sum + (0, dates_1.minutesBetween)(session.start, session.end || now), 0);
...
function calculateProductivityScore(record, tasks) {
    ...
    const breakScore = breakRatio === 0
        ? 0.5
        : breakRatio <= IDEAL_BREAK_RATIO * 2
            ? 1
            : Math.max(0, 1 - (breakRatio - IDEAL_BREAK_RATIO * 2) * 5);
    return Math.min(100, Math.max(0, Math.round(workHoursScore * 35 + taskScore * 35 + deepWorkRatio * 20 + breakScore * 10)));
}
function detectBurnoutRisk(record, allRecords, tasks) {
    ...
    const last5 = allRecords
        .filter((attendance) => attendance.date < record.date && attendance.date >= (0, dates_1.offsetDate)(record.date, -5))
        .map((attendance) => computeRecord(attendance).totalWorkMinutes);
...
function generateInsights(allRecords) {
    ...
    const avgCheckInMinutes = Math.round(checkInMinutes.reduce((sum, value) => sum + value, 0) / Math.max(checkInMinutes.length, 1));
    ...
    for (let index = 0; index < sortedDates.length; index += 1) {
        if (sortedDates[index] === (0, dates_1.offsetDate)(today, -index)) {
            currentStreak += 1;
        }
        else {
            break;
    ...
    for (const date of [...sortedDates].reverse()) {
        if (!previousDate || (0, dates_1.offsetDate)(previousDate, 1) === date) {
            tempStreak += 1;
    ...
    const last30 = Array.from({ length: 30 }, (_, index) => (0, dates_1.offsetDate)(today, -index));
...
function generateCalendarData(allRecords, months = 2) {
    ...
    for (let monthOffset = months - 1; monthOffset >= 0; monthOffset -= 1) {
        ...
        for (let day = 1; day <= daysInMonth; day += 1) {
            ...
            const record = allRecords.find((attendance) => attendance.date === dateStr);
            let status = 'no-data';
            if (record) {
                status = determineStatus(record);
            }
            else if (dateStr <= (0, dates_1.todayStr)()) {
                status = 'absent';
            }
            days.push({
                date: dateStr,
...
function generateWeeklyStats(allRecords) {
    const today = (0, dates_1.todayStr)();
    return Array.from({ length: 7 }, (_, index) => {
        const date = (0, dates_1.offsetDate)(today, -(6 - index));
        const record = allRecords.find((attendance) => attendance.date === date);
...
function addTimeline(record, action) {
    record.timeline.push({
        time: new Date().toISOString(),
        action,
    });
}
async function updateTodayRecord(userId, updater) {
    const tasks = await (0, task_service_1.listTasks)(userId);
    const history = await getAllRecords(userId);
    const record = history.find((attendance) => attendance.date === (0, dates_1.todayStr)()) ??
        (await getOrCreateToday(userId));
    if (!history.some((attendance) => attendance.attendanceId === record.attendanceId)) {
        history.push(record);
    }
    updater(record);
...
async function startBreakForUser(userId, type) {
    return updateTodayRecord(userId, (record) => {
        if (!record.checkIn) {
        ...
        record.breaks.push({
            breakId: (0, node_crypto_1.randomUUID)(),
...
async function endBreakForUser(userId) {
    return updateTodayRecord(userId, (record) => {
        const activeBreak = record.breaks.find((breakEntry) => !breakEntry.end);
        if (!activeBreak) {
...
async function startDeepWorkForUser(userId) {
    return updateTodayRecord(userId, (record) => {
        if (!record.checkIn) {
            throw new Error('Not checked in');
        }
...
async function endDeepWorkForUser(userId) {
    return updateTodayRecord(userId, (record) => {
        const activeSession = record.deepWorkSessions.find((session) => !session.end);
        if (!activeSession) {
...
async function getAttendanceCalendar(userId, months = 2) {
    const records = await getAllRecords(userId);
    return generateCalendarData(records, months);
}
async function getAttendanceInsights(userId) {
    const records = await getAllRecords(userId);
    const tasks = await (0, task_service_1.listTasks)(userId);
    const insights = generateInsights(records);
    const todayRecord = records.find((record) => record.date === (0, dates_1.todayStr)());
